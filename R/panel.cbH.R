...
#'
#' @param lwd.bands Largura para linhas que delimitam o polígono quanto
#'     \code{cty = "bands"} é usado.
#'
...
panel.cbH <- function(x, y, ly, uy,
    ...
    if (cty == "bands") {
        ...
        panel.lines(xo, lyo, lty = 2, lwd = lwd.bands, col = col.line)
        panel.lines(xo, uyo, lty = 2, lwd = lwd.bands, col = col.line)
    }
    if (cty == "bars") {
